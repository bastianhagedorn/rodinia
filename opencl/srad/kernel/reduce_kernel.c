...
reduce(int d_Ne,  // number of elements in array
  ...
  int nf = NUMBER_THREADS - (gridDim.x * NUMBER_THREADS -
                             d_no); // number of elements assigned to last block
  int df = 0;                       // divisibility factor for the last block

  // statistical
  __shared__ fp d_psum[NUMBER_THREADS]; // data for block calculations allocated
  ...
  if (nf == NUMBER_THREADS) {
    ...
    if (bx != (gridDim.x - 1)) { //
      ...
      if (tx == (df - 1)) { //
        ...
        for (i = (bx * NUMBER_THREADS) + df; i < (bx * NUMBER_THREADS) + nf;
             i++) { //
