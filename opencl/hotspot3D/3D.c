...
void usage(int argc, char **argv) {
	...
	fprintf(stderr, "\t<tempFile>  - name of the file containing the initial temperature "
			"values of each cell\n");
...
int main(int argc, char **argv) {
	...
	err = clEnqueueWriteBuffer(commands, d_a, CL_TRUE, 0, sizeof(float) * count, tIn, 0, NULL,
				   NULL);
	if (err != CL_SUCCESS) {
	...
	err = clEnqueueWriteBuffer(commands, d_b, CL_TRUE, 0, sizeof(float) * count, pIn, 0, NULL,
				   NULL);
	if (err != CL_SUCCESS) {
	...
	err = clEnqueueWriteBuffer(commands, d_c, CL_TRUE, 0, sizeof(float) * count, tempOut, 0,
				   NULL, NULL);
	if (err != CL_SUCCESS) {
	...
	for (j = 0; j < iterations; j++) {
		...
		err = clEnqueueNDRangeKernel(commands, ko_vadd, 2, NULL, global, local, 0, NULL,
					     &events[j]);
		if (err) {
	...
	err = clEnqueueReadBuffer(commands, d_c, CL_TRUE, 0, sizeof(float) * count, tempOut, 0,
				  NULL, NULL);
	if (err != CL_SUCCESS) {
	...
	computeTempCPU(pIn, tempCopy, answer, numCols, numRows, layers, Cap, Rx, Ry, Rz, dt,
		       amb_temp, iterations);
